      ...
      ? employees.flatMap(emp =>
          Array.from({ length: monthDiff }).map((_, i) => {
            ...
            return {
              ...
              queryFn: () => payrollService.generatePayroll(emp.id, month, year),
              enabled: !!emp.id,
            };
          })
        )
  ...
  const totalSalary = useMemo(() => {
    ...
    return employees.reduce((total, emp) => {
      ...
      for (let i = 0; i < monthDiff; i++) {
        ...
        const payrollQuery = queries.find(q => q.data?.empId === emp.id && q.data.month === month && q.data.year === year);
        if (!payrollQuery || !payrollQuery.data) continue;

        const payroll = payrollQuery.data;
        empTotal += (payroll.netSalary || 0) + (payroll.epfEmployer || 0) + (payroll.etf || 0);
      }

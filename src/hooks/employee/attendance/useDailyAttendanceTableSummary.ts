...
export const useDailyAttendanceTableSummary = (date : string,searchQuery : string) => {
  ...
  const dailyAttendance = attendanceRecords?.map((record) => {
    ...
    return {
      ...record,
      employeeName: employee ? `${employee.empFirstName} ${employee.empLastName}` : null,
    };
  }) || [];

  const filteredAttendance = dailyAttendance.filter((record) => {
    const employeeName = record.employeeName ? record.employeeName.toLowerCase() : "";

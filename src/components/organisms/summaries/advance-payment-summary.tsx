...
export default function EmpAdvancePaymentSummary(props: {
  empId: number;
  year: number;
  month: number;
}) {
  const { data: fetchedAdvancePayments } = useGetAdvanceByEmpAndMonthYear(
    props.empId,
    props.year.toString(),
    props.month.toString()
  );
    ...
    <div className="space-y-6 p-6 bg-card rounded-2xl shadow-md lg:col-span-2 flex flex-col">
      ...
      <div>
        ...
        <h2 className="text-position-text">
          Total Advance Payments: Rs.
          {fetchedAdvancePayments?.reduce(

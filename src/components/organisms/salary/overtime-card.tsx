      ...
      <div className="grid grid-cols-1  gap-6 items-stretch">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
          <div className="space-y-3 ">
            <h4 className="text-position-text font-light w-full sm:w-32 md:w-40 text-sm sm:text-base">
              Double OT
            </h4>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <ReadOnlyField
                label="Double OT Hours"
                value={`${data.doubleOTHours}`}
              />
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <ReadOnlyField
                label="Double OT Amount (per hour)"
                value={`Rs.${data.doubleOTRate}`}
              />
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <ReadOnlyField
                label="Total Double OT"
                value={`Rs.${data.doubleOTAmount}`}
              />
            </div>
          </div>
          ...
          <div className="space-y-3 ">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <ReadOnlyField
                ...
                value={`${data.singleOTHours}`}
              />
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <ReadOnlyField
                label="Single OT Amount (per hour)"
                value={`Rs.${data.singleOTRate}`}
              />
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <ReadOnlyField
                label="Total Single OT"
                value={`Rs.${data.singleOTAmount}`}
              />
            </div>
          </div>
        </div>

    ...
    <div className="flex flex-col lg:flex-row gap-8 lg:pl-10">
      ...
      <div className="w-full lg:w-[280px] order-1 lg:order-2">
        <ProfileImageUploader
            imageUrl={
              imagePreview ?? formData?.imgUrl ?? employee?.imgUrl ?? ""
            }
            editable={isEditing}
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <BaseInputField
              ...
              readonly={!isEditing}
              onChange={(v) => handleChange("gender", v as "MALE" | "FEMALE" | "OTHER")}
            />
            <BaseInputField
              label="Age"
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <BaseCheckboxField
              ...
              value={formData?.commissionEligible ?? employee?.commissionEligible ?? false}
              disabled={!isEditing}
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <BaseInputField
              ...
              value={formData?.employeeBankDetails?.accountNumber ?? employee?.employeeBankDetails?.accountNumber ?? ""}
              readonly={!isEditing}
              onChange={(v) => handleBankChange("accountNumber", v)}
            />
            <BaseInputField
              label="Account Holder Name"
              value={formData?.employeeBankDetails?.accountHolderName ?? employee?.employeeBankDetails?.accountHolderName ?? ""}
              readonly={!isEditing}
              onChange={(v) => handleBankChange("accountHolderName", v)}
            />
            <BaseInputField
              label="Bank Name"
              value={formData?.employeeBankDetails?.bankName ?? employee?.employeeBankDetails?.bankName ?? ""}
              readonly={!isEditing}
              onChange={(v) => handleBankChange("bankName", v)}
            />
            <BaseInputField
              label="Branch Name"
              value={formData?.employeeBankDetails?.branchName ?? employee?.employeeBankDetails?.branchName ?? ""}
              readonly={!isEditing}

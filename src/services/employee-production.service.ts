...
export const employeeProductionService = {
  async getAll(): Promise<EmployeeProductionResponse[]> {
    const response = await apiClient.request<EmployeeProductionResponse[]>(
      API_ENDPOINTS.EMPLOYEE_PRODUCTION.GET_ALL,
      { method: "GET" }
    );
  ...
  async getByEmployee(id: number): Promise<EmployeeProductionResponse[]> {
    const response = await apiClient.request<EmployeeProductionResponse[]>(
      API_ENDPOINTS.EMPLOYEE_PRODUCTION.GET_BY_EMPLOYEE(id),
      { method: "GET" }
    );
    return z.array(EmployeeProductionResponseSchema).parse(response);
  },
  ...
  ): Promise<EmployeeProductionRequest> {
    const response = await apiClient.request<EmployeeProductionRequest>(
      API_ENDPOINTS.EMPLOYEE_PRODUCTION.ADD,
      {
        method: "POST",
        data,
      }
  ...
  async updateProductionRecord(
    id: number,
    data: Partial<EmployeeProductionRequest>
  ): Promise<EmployeeProductionResponse> {
    const response = await apiClient.request<EmployeeProductionRequest>(
      API_ENDPOINTS.EMPLOYEE_PRODUCTION.UPDATE(id),
      {
        method: "PUT",
        data,
      }
  ...
  async deleteProductionRecord(id: number): Promise<void> {
    await apiClient.request<void>(
      API_ENDPOINTS.EMPLOYEE_PRODUCTION.DELETE(id),
      { method: "DELETE" }
    );
  },
  ...
  async getByDateRange(
    ...
    const response = await apiClient.request<EmployeeProductionResponse[]>(
      API_ENDPOINTS.EMPLOYEE_PRODUCTION.GET_BY_DATE_RANGE(startDate, endDate),
      { method: "GET" }
    );
    return z.array(EmployeeProductionResponseSchema).parse(response);
  },

  async getByDateRangeAndEmployee(
    id: number,
    year: string,
    month: string
  ): Promise<EmployeeProductionResponse[]> {
    ...
    const response = await apiClient.request<EmployeeProductionResponse[]>(
      API_ENDPOINTS.EMPLOYEE_PRODUCTION.GET_BY_DATE_RANGE_EMPLOYEE(
        id,
        startDate,
        endDate
      ),
